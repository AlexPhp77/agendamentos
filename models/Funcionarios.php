...
class Funcionarios extends Conexao{
    ...
    public function permissoes(){  
	        ...
	        $sql->bindValue(':id', $_SESSION['logadoFuncionario']);
	        $sql->execute();        
    ...
    private function cadastrar(){    	
    	...
    	if($this->verificarEmail()){
    		...
    		if($this->verificarCpf()){
		    	...
		    	$sql = $this->pdo->prepare("INSERT INTO funcionarios SET permissoes = :permissoes, nome = trim(:nome), idade = trim(:idade), sexo = :sexo, cpf = trim(:cpf), email = trim(:email), telefone = trim(:telefone), senha = md5(trim(:senha)), estado = trim(:estado), cidade = trim(:cidade), cep = trim(:cep), rua = trim(:rua), numero = trim(:numero)");	    	
	...
	public function login($email, $senha){			 
		...
		$sql = $this->pdo->prepare("SELECT * FROM funcionarios WHERE email = :email AND senha = :senha");
		...
		if($sql->rowCount() > 0){
			$dado = $sql->fetch();
			$_SESSION['logadoFuncionario'] = $dado['id'];
		    //return true; 
			?>
	...
	public function deletar($id){
		...
		if($dados['permissoes'] == 'ADMINISTRADOR'){		
        	...
        	$sql = $this->pdo->prepare('DELETE funcionarios, endereco FROM usuarios INNER JOIN endereco ON endereco.id_usuario = usuarios.id WHERE usuarios.id = :id_logado');
	...
	public function pesquisar($texto){
		...
		$sql = $this->pdo->prepare("SELECT id, nome FROM funcionarios WHERE nome LIKE :texto OR cpf LIKE :texto OR email = :texto");
	...
	public function getPaciente($id){
        $dados = array();
		$sql = $this->pdo->prepare("SELECT usuarios.id, usuarios.nome, usuarios.idade, usuarios.sexo, usuarios.cpf, usuarios.email, usuarios.telefone, endereco.estado, endereco.cidade, endereco.cep, endereco.rua, endereco.numero FROM funcionarios INNER JOIN endereco ON endereco.id_usuario = usuarios.id WHERE usuarios.id= :id");
		$sql->bindValue(':id', $id);
		$sql->execute();
	...
	public function recuperarSenha(){
        $sql = $this->pdo->prepare("SELECT id, email FROM funcionarios WHERE email = :email");
        $sql->bindValue(':email', $this->email);
        $sql->execute();
    ...
    public function redefinirSenha($cod){
        ...
        if($sql->rowCount() > 0){
            ...
            $sql = $this->pdo->prepare("UPDATE funcionarios SET senha = md5(:senha) WHERE id = :id");
            $sql->bindValue(':senha', $this->senha);
            $sql->bindValue(':id', $id);
            $sql->execute();
